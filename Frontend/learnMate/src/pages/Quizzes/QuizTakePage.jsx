...
import { ChevronLeft,ChevronRight,CheckCircle2, Option } from 'lucide-react'
...
const QuizTakePage = () => {
  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      try {
        const response = await quizService.getQuizeById(quizId);
       ...
       <div className='inline-flex items-center gap-2 px-4 py-2 bg-linear-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl mb-6'>
               ...
               return (
                  <label 
                   ...
                   className={`group relative flex items-center p-3 border-2 rounded-xl cursor-pointer transition-all duration-200 
                    ${isSelected ? 'border-emerald-500 bg-emerald-50 shadow-lg shadow-emerald-500/10'
         ...
         {/* Navigation Button */}
          <div className='flex items-center justify-between gap-4'>
            ...
            variant='secondry'
            >
              <ChevronLeft className='w-4 h-4 group-hover:-translate-0.5 transition-transform duration-200' strokeWidth={2.5}/>
              Previous
            </Button>
            ...
            {currentQuestionIndex === quiz.questions.length - 1 ? (
              ...
              >
                ...
                <div className='absolute inset-0 bg-linear-to-r from-white/30 via-white/20 to-white/0 -translate-x-full group-hover:translate-x-full transition-transform duration-700'/>
              </button>
            ):(
              <Button
