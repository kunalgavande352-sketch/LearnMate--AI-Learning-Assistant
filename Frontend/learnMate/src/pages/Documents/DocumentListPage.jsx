...
const fetchDocument = async () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setDocuments((prev) =>
        prev.filter((d) => d._id !== selectedDocs._id)
      );
        ...
        <div className="flex items-center justify-center min-h-[400px]">
         <div className="text-center max-w-md">
           ...
           <p className="text-sm text-slate-500 mb-6">
            Get started by uploading your first PDF document to being learning.
           </p>
           <button
            ...
            <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Confirm deletion for "{selectedDocs.title}".
                  </p>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
           <div className="relative w-full max-w-lg bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-2xl shadow-slate-900/20 p-8">
            {/* Close button */}
             <button 
             onClick={() => setIsUploadModalOpen(false)}
             className="absulate top-6 right-6 w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-600  hover:bg-slate-100 transition-all duration-200"
             >
                ...
                <form onSubmit={handleUpload} className="sapce-y-5">
                  {/* Title Input */}
                   <div className="space-y-2">
                     <label className="block text-xl font-semibold text-slate-700 uppercase tracking-wide">
                      Document Title
                     </label>
                   ...
                   {/* Action Button */}
                    <div className="flex gap-3 pt-2">
                       ...
                       type="submit"
                       disabled={uploading}
                       className="flex-1 h-11 px-4 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/50 disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98]"
                       >
                        {uploading ? (
